...
import com.acmebank.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleSystemException(HttpServletRequest req, Exception e) {
		CustomException sysEx = (CustomException) e;

		String errorMsg = (e.getMessage() == null) ? e.getClass().getSimpleName() : e.getMessage();
		log.error("[" + sysEx.getMessage() + "] " + sysEx.getMessage());

		ErrorResponse response = new ErrorResponse();
		response.setErrorCode(sysEx.getMessage());
	...
	public ResponseEntity<ErrorResponse> handleBusinessException(HttpServletRequest req, Exception e) {
		AccountNotExistException businessEx = (AccountNotExistException) e;

		String errorMsg = (e.getMessage() == null) ? e.getClass().getSimpleName() : e.getMessage();
		log.error("[" + businessEx.getMessage() + "] " + businessEx.getMessage());

		ErrorResponse response = new ErrorResponse();
		response.setErrorCode(businessEx.getMessage());
		response.setErrorMessage(errorMsg);
